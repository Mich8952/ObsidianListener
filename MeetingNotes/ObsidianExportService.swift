...
final class DefaultObsidianExportService: ObsidianExportService {
    ...
    private func exportLocked(_ request: ExportRequest) throws -> URL {
        ...
        let noteURL = collisionFreeURL(in: request.folder, stem: stem, extension: "md")
        var audioName: String?
        if request.keepOriginalAudio, let path = request.session.mixedWAVPath {
            let audioURL = noteURL.deletingPathExtension().appendingPathExtension("wav")
    ...
    private func collisionFreeURL(in folder: URL, stem: String, extension ext: String) -> URL { var n = 1; while true { let suffix = n == 1 ? "" : " (\(n))"; let url = folder.appendingPathComponent(stem + suffix).appendingPathExtension(ext); if !FileManager.default.fileExists(atPath: url.path) { return url }; n += 1 } }
    func renderMarkdown(session: RecordingSession, audioFileName: String?, saveTranscript: Bool, saveSummary: Bool) -> String {
        ...
        if saveSummary, let s = session.summary { out += "\n## Summary\n\n\(s.summary)\n\n## Decisions\n\n\(bullets(s.decisions))\n\n## Action Items\n\n\(s.actionItems.map { "- \($0.task)" + ($0.owner.map { " — \($0)" } ?? "") }.joined(separator: "\n"))\n\n## Open Questions\n\n\(bullets(s.openQuestions))\n\n## Technical Details\n\n\(bullets(s.technicalDetails))\n" }
        if saveTranscript { out += "\n## Transcript\n\n" + session.transcript.segments.map { "[\(clock($0.start))] \($0.text)" }.joined(separator: "\n") + "\n" }
        return out
    }
